// Joao Oliveira — Layer 2: Organic Branching Nodes
// An organic growing root/branch system that reacts to music
// Branches grow, curve with Perlin noise, and connect nearby nodes with a proximity web
...
class LayerJO2 extends Layer {
  ...
  // Track overall amplitude and decaying beat
  float smoothAmp = 0;
  float beatDecay = 0;
  ...
  LayerJO2(color c) {
    ...
    // Start with 5 root nodes at random positions
    for (int i = 0; i < 5; i++) {
  ...
  void update(float t, float bass, float mid, float treble,
    ...
    // Growth is driven by bass and mid frequencies
    growthAge += 0.2 + bass * 4 + mid * 2;

    // Update all nodes and remove dead ones
    ...
    // Maintain desired node count based on growth age
    int desired = min((int)(growthAge * 0.13) + 5, MAX_NODES);
    while (nodes.size() < desired) { if (!spawnNode()) break; }

    // On strong beat, spawn bursts of extra child branches
    if (beatDecay > 0.3) {
    ...
    // Compute visibility using a smooth step function
  ...
  // Spawn a new child node from a random existing node
  boolean spawnNode() {
    for (int i = 0; i < 40; i++) {
      Node p = nodes.get((int)random(nodes.size()));
      if (p.children < 4 && p.growT > 0.3) {
        ...
        for (int j = 0; j < arm; j++) {
          ...
          Node c = new Node(p.endX, p.endY, a, max(l, 35));
          nodes.add(c);
          p.children++;
        }
        return true;
  ...
  void drawLayer(PGraphics g) {
    ...
    // Draw the organic curved branches
    for (Node n : nodes) n.draw(g, currentT, bassS, trebS, visibility, smoothAmp);

    // Casey Reas Proximity Web — lines connecting nearby nodes
    g.colorMode(RGB, 255);
    g.strokeWeight(1.0);
  ...
  // Node representing a growing branch segment
  class Node {
    float x, y, endX, endY;
    ...
    // Grow the node, adding Perlin noise drift and responding to mid frequency
    void grow(float t, float mid) {
      ...
      // Drift using sine waves for organic curving
    ...
    // Draw the node as a bezier curve with HSB coloring and circular joints
    void draw(PGraphics g, float t, float bass, float treb, float vis, float amp) {
      ...
      // Draw circular joints at branch tips
      if (tailT < 0.8) {
